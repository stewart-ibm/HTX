...
/*                                                                        */
/* htxubuntu src/htx/usr/lpp/htx/lib/htxsyscfg64/htxsyscfg_new.c 1.15.1.25 */
/*                                                                        */
...
/*  @(#)85        1.15.1.19  src/htx/usr/lpp/htx/lib/htxsyscfg64/htxsyscfg_new.c, htx_libhtxsyscfg64, htxfedora 5/19/15 01:28:11									
...
int init_syscfg(void)
{
    ...
    if (shm_id_htxsyscfg == -1){
		if(misc_htx_data != NULL){
			...
			printf("shm id creation failed with errno=%d \n", errno);
		}
		return(101);
    }
    ...
    if ( r ) {
        if(misc_htx_data != NULL){
            ...
            printf("update_syscfg() failed with rc=%d\n", r);
        }
		return(103);	
    }
	return 0;
...
int init_rwlocks(void){
        ...
        if ( lockinit1 == (EAGAIN || ENOMEM) ) {
            ...
            if(lockinit1_1 !=0){
                if(misc_htx_data != NULL){
                    ...
                    printf("rwlock attr initialisation failed with rc=%d\n", lockinit1);
                }
            }
        }
        else{
    ...
    if (lockinit1!=0  ) {
        if(misc_htx_data != NULL){
            ...
            printf("haring rwlock attr across processes failed with rc=%d\n", lockinit1);
        }
	}
    ...
    if (lockinit1!=0  ) {
        if ( lockinit1 == EBUSY ){
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_lpar.rw_lock_init()1 failed with rc=%d\n", lockinit1);
            }
        }
            ...
            if(lockinit1_1 !=0){
                if(misc_htx_data != NULL){
				    ...
				    printf("global_ptr->global_lpar.rw_lock() failed in retry with rc=%d\n", lockinit1);
	            }
			}
		}
        else{
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_lpar.rw_lock() failed with rc=%d\n", lockinit1);
            }
        }
    }
    ...
    if (lockinit2!=0  ) {
        if ( lockinit2 == EBUSY ){
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_memory.rw_lock_init() failed with rc=%d\n", lockinit2);
            }
        }
        else if ( lockinit1 == (EAGAIN || ENOMEM) ) {
            usleep(10);
    ...
    if (lockinit3!=0  ) {
        if ( lockinit3 == EBUSY ){
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_cache.rw_lock_init() failed with rc=%d\n", lockinit3);
            }
        }
        else if ( lockinit1 == (EAGAIN || ENOMEM) ) {
            usleep(10);
    ...
    if (lockinit4!=0  ) {
        if ( lockinit4 == EBUSY ){
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->syscfg.rw_lock_init() failed with rc=%d\n", lockinit4);
            }
        }
        else if ( lockinit1 == (EAGAIN || ENOMEM) ) {
            usleep(10);
    ...
    if (lockinit5!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->system_cpu_map.stat.rw_lock_init() failed with rc=%d\n", lockinit5);
		}
    }
    if (lockinit5!=0  ) {
        if ( lockinit5 == EBUSY ){
    ...
    if (lockinit6!=0  ) {
        if ( lockinit6 == EBUSY ){
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_core.rw_lock_init() failed with rc=%d\n", lockinit6);
            }
        }
        else if ( lockinit1 == (EAGAIN || ENOMEM) ) {
            usleep(10);
...
int attach_to_syscfg(void)
{
    ...
    if (shm_id_htxsyscfg == -1)/* perror ("Creation ");*/{
        if(misc_htx_data != NULL){
            ...
            printf("inside attach_to_syscfg, shm_id creation failed= %d \n",errno);
        }
		return(107); 
    }
	...
	if(global_ptr == (void *)(-1)) {
		if(misc_htx_data != NULL){
			...
			printf("shmat failed inside attach_to_syscfg with errno=%d", errno);
		}
		return(108);
	}
	return 0;
    ...
    if ( temp_shm_key == -1 && errno == EEXIST ) {
		...
		if (lock1!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_lpar.rw_lock_destroy() failed with rc=%d\n", lock1);
			}
		}
		...
		if (lock2!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_memory.rw_lock_destroy() failed with rc=%d\n", lock2);
	        }
		}
		...
		if (lock3!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_cache.rw_lock_destroy() failed with rc=%d\n", lock3);
			}	
		}
		...
		if (lock4!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->syscfg.rw_lock_destroy() failed with rc=%d\n", lock4);
			}
		}
		...
		if (lock5!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->system_cpu_map.stat.rw_lock_destroy() failed with rc=%d\n", lock5);
			}
		}
		...
		if (lock6!=0  ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_core.rw_lock_destroy() failed with rc=%d\n", lock6);
	        }
		}
		...
		if (shmdt_check ==  -1){
			if(misc_htx_data != NULL){
				...
				printf("shmdt failed with errno=%d", errno);
			}
		}
		...
		if(i==-1){
			if(misc_htx_data != NULL){
				...
				printf("inside detach_syscfg, shmctl failed= %d ",errno);
			}
		}
	    ...
	    if ( fp != NULL ) {
    	    while( !feof(fp) && !ferror(fp)) {
                ...
                if(misc_htx_data != NULL){
                    ...
                    printf("syscfg %d:Failed popen for command %s ,errno=%d,re-try count=%d\n",__LINE__,cmd,errno,count);
                }
                count++;
    ...
    if ( sys_mounted == 0 ) {
        if (mkdir("/sys", 0777)) {
			if(misc_htx_data != NULL){
				...
				perror("syscfg:/sys mkdir failed");
			}
            return -1;
        }
        ...
        if (mount("sys", "/sys", "sysfs", MS_NOATIME, NULL)){
			...
			hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
				...
				perror("syscfg:/sys mount failed");
			}
            return -1;
        }
    }
	...
	*t =(global_ptr->global_lpar.cpus);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside phy_logical_virt_cpus failed with errno=%d\n",rc2);
		}
	}
    return 0;
}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_lpar_details failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("global_ptr->global_lpar.rw_lock() in get_lpar_details  failed in retry with rc=%d\n",rc1_1); 
			    }
			}
        }
		else{
            if(misc_htx_data != NULL){
                ...
                printf("global_ptr->global_lpar.rw_lock() in get_lpar_details failed with rc=%d\n",rc1); 
            }
		}
    }
	*t=(global_ptr->global_lpar);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_lpar_details failed with errno=%d\n",rc1);
		}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_smt_details failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_smt_details failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
    ...
    *t =(global_ptr->global_core.smtdetails);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_smt_details failed with errno=%d\n",rc1);
		}
...
char ch, freq_str[20];
    ...
    if ( fp != NULL ) {
		...
		hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
			...
			printf("syscfg:popen failed for /proc/cpuinfo\n");
		}
        return (-1);
    }
...
long long get_timebase(void)
{
    ...
    if ( fp != NULL ) {
		...
		hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
			...
			printf("popen failed for /proc/cpuinfo\n");
		}
        return (-1);
    }
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_core_details failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_core_details failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*t = (global_ptr->global_core);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_core_details failed with errno=%d\n",rc1);
		}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_memory_size failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_memory_size failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*t=(global_ptr->global_memory.mem_size);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_memory_size failed with errno=%d\n",rc1);
		}
	}
    return 0;
}
...
int get_page_size(htxsyscfg_pages_t *t)
{
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_page_size failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_page_size failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_page_size failed with errno=%d\n",rc1);
		}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_memory_details failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_memory_details failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*t = (global_ptr->global_memory);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_memory_details failed with errno=%d\n",rc1);
		}
;
	}
    return 0;
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside L1cache failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside L1cache failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*t=(global_ptr->global_cache);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside L1cache failed with errno=%d\n",rc1);
		}
	}
    return 0;   
}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside L2L3cache failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
	...
	*t=(global_ptr->global_cache);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside L2L3cache failed with errno=%d\n",rc1);
		}
	}
    return 0;   
}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_env_details failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_env_details failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*e =(global_ptr->global_lpar.env_details);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_env_details failed with errno=%d\n",rc1);
		}
	}
	return;
}
	...
	printf("pir = %d\n",pir);
        ...
        if(rc == -1) {
                if(misc_htx_data != NULL){
                    ...
                    printf("syscfg:unbind failed for cpu:%d ,rc=%d\n",i,rc);
                }
            return(-1);
        }
    }
            ...
            if(node == -1 || core == -1 || chip == -1){
                   if(misc_htx_data != NULL){
                    ...
                    hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
                        ...
                        printf("syscfg:something went wrong,node=%d,chip=%d,core=%d\n",node,chip,core);
                    }
                    return -1;
            }
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_hardware_config failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
			...
			if(rc1_1!=0){
	            if(misc_htx_data != NULL){
		            ...
		            hxfmsg(misc_htx_data, -1, HTX_HE_SOFT_ERROR, msg);
			         ...
			         printf("lock inside get_hardware_config failed in retry with errno=%d\n",rc1);
			    }
			}
        }
		else{
	...
	*sys_conf = (global_ptr->syscfg);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_hardware_config failed with errno=%d\n",rc1);
		}
	}
    return 0;
}
...
int get_cpus_in_node(int node_no,signed int *cpus_in_node)
{
    ...
    if( node_no > global_ptr->syscfg.num_nodes ) {
		...
		hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
			...
			printf("syscfg:node num:%d passed is greater than total nodes:%d\n",node_no,global_ptr->syscfg.num_nodes);
		}
        return (-1);
    }
...
int get_cpus_in_chip(int chip_no,signed int *cpus_in_chip)
{
    ...
    if( chip_no>MAX_CHIPS ){
		if(misc_htx_data != NULL){
			...
			printf("syscfg:chip num:%d passed is greater than max chips:%d\n",chip_no,MAX_CHIPS);
		}
        return (-1);
	}
    ...
    if (rc1!=0  ) {
        if ( rc1 == EDEADLK ) {
            if(misc_htx_data != NULL){
                ...
                printf("lock inside get_hardware_stat failed with errno=%d\n",rc1);
            }
        }
        else if ( rc1 == EAGAIN ) {
			usleep(10);
	...
	*sys_stat = (global_ptr->system_cpu_map.stat);
	...
	if (rc2 !=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("unlock inside get_hardware_stat failed with errno=%d\n",rc1);
		}
	}
    return 0;
}
...
int get_cpus_in_core(int core_no,signed int *cpus_in_core)
{
    ...
    if( core_no>MAX_CORES ){
		...
		hxfmsg(misc_htx_data, 0, HTX_HE_INFO, msg);
			...
			printf("syscfg:core num:%d passed is greater than max cores:%d\n",core_no,MAX_CORES);
		}
...
unsigned int getPir(int cpu_number)
{
    ...
    if (FP == NULL)
    {
		...
		if(misc_htx_data != NULL){
			...
			printf("syscfg:Miscex is not loaded and Sysfs doesnot have pir file,exiting \n");
		}
        return(-1);
		#endif
         pir = (unsigned int)get_cpu_id_sysfs(cpu_number); /* misc is not loaded, read from sysfs*/
    }
    else
    {
        fclose(FP);
        pir = (unsigned int) get_cpu_id(); /* misc is loaded so read it from misc */
    }
    return pir;
}
    ...
    if ((misc_htx_data != NULL)  && ((strcmp (misc_htx_data->run_type, "OTH") == 0))) {
        ...
        if ( rc ){
			if(misc_htx_data != NULL){
				...
				printf("Error in repopulate_syscfg while run from cmd line. errno=%d\n", rc);
	        }
		}
		return(rc);
    }
		...
		if( global_ptr == NULL ){
			...
			if ( rc ){ 
	            if(misc_htx_data != NULL){
					...
					printf("Error in repopulate_syscfg. errno=%d\n", rc);
	            }
				return(rc);
			}
		}
            ...
            if(misc_htx_data != NULL){
                ...
                printf("Error in shmget::shm not existing with errno=%d\n", errno);
            }
			return(109);
		}
				...
				if(misc_htx_data != NULL){
					...
					printf("Error in shmget when global_ptr is NULL. errno=%d\n", errno);
				}
				return(110);
			}
		}
...
int update_syscfg(void)
{
		...
		if ( temp_shm_key == -1 && errno == EEXIST ) {
			...
			if(rc){
				if(misc_htx_data != NULL){
					...
					printf("attach_to_syscfg failed inside update_syscfg with errno=%d\n",rc);
				}
				return(rc);
			}
        }
	...
	if ( r1 ) {
		if(misc_htx_data != NULL){
			...
			printf("phy_logical_virt_cpus_update() failed with rc=%d\n", r1);
		}
    }
	...
	if ( r2 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_env_details_update() failed with rc=%d\n", r2);
		}
    }
	...
	if ( r3 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_lpar_details__update() failed with rc=%d\n", r3);
		}
    }
	...
	if (lock2 != 0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->global_lpar.rw_unlock() failed with rc= %d ", lock2);
		}
	}
	...
	if (lock3!=0  ) {
		if ( lock3 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_memory.rw_lock3() failed with rc=%d\n", lock3);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if ( r4 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_memory_size__update() failed with rc=%d\n", r4);
		}
    }
    ...
    if ( r5 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_page_size__update() failed with rc=%d\n", r5);
		}
    }
    ...
    if ( r18 ) {
        if(misc_htx_data != NULL){
            ...
            printf("get_page_details() failed with rc=%d\n", r18);
        }
    }
	...
	if ( r7 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_memory_details_update() failed with rc=%d\n", r7);
		}
    }
	...
	if (lock4!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->global_memory.rw_unlock() failed with rc=%d\n", lock4);
		}
    }
	...
	if (lock5!=0  ) {
		if ( lock5 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_cache.rw_unlock() failed with rc=%d\n", lock5);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if ( r8 ) {
		if(misc_htx_data != NULL){
			...
			printf("L1cache_update() failed with rc = %d \n", r8);
		}
    }
	r9=L2L3cache_update();
	if ( r9 ) {
		if(misc_htx_data != NULL){
			...
			printf("L2cache_update() failed with rc=%d\n", r9);
		}
    }
	...
	if (lock7!=0  ) {
		if ( lock7 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_syscfg.rw_lock() failed with rc=%d\n", lock7);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if (lock18!=0  ) {
		if ( lock1 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->system_cpu_map.rw_lock() inside get_hardware_config_update failed with rc=%d\n", lock18);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if ( r11 ) {
		if(misc_htx_data != NULL){
			...
			printf("get_hardware_config_update() failed with rc=%d\n", r11);
		}
    }
    ...
    if (lock19!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->system_cpu_map.rw_unlock() failed with rc=%d\n", lock19);
		}
    }
	...
	if (lock8!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->syscfg.rw_unlock() failed with rc=%d\n", lock8);
		}
	}
	...
	if (lock9!=0  ) {
		if ( lock9 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->system_cpu_map.stat.rw_lock() failed with rc=%d\n", lock9);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if ( r12) {
		if(misc_htx_data != NULL){
			...
			printf("get_hardware_stat_update() failed with rc=%d\n", r12);
		}
     }
	...
	if (lock12!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->system_cpu_map.stat.rw_unlock() failed with rc=%d\n", lock12);
		}
	}
	...
	if (lock13!=0  ) {
		if ( lock13 == EDEADLK ) {
			if(misc_htx_data != NULL){
				...
				printf("global_ptr->global_core.rw_lock() failed with rc=%d\n", lock13);
			}
		}
		else{
			if(misc_htx_data != NULL){
	...
	if ( r17) {
		if(misc_htx_data != NULL){
			...
			printf("get_hw_smt() failed with rc=%d\n", r17);
		}
    }
	...
	if ( r15) {
		if(misc_htx_data != NULL){
			...
			printf("get_min_smt() failed with rc=%d\n", r15);
		}
    }
	...
	if ( r16) {
		if(misc_htx_data != NULL){
			...
			printf("get_max_smt() failed with rc=%d\n", r16);
		}
    }
	...
	if ( r13) {
		if(misc_htx_data != NULL){
			...
			printf("get_smt_details_update() failed with rc=%d\n", r13);
		}
    }
	...
	if ( r14) {
		if(misc_htx_data != NULL){
			...
			printf("get_core_details_update() failed with rc=%d\n", r14);
		}
    }
	...
	if (lock14!=0  ) {
		if(misc_htx_data != NULL){
			...
			printf("global_ptr->global_core.rw_unlock() failed with rc=%d\n", lock14);
		}
	}
