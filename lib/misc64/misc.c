...
int retry_open_calls(FILE** f_ptr,const char* str,const char* mode){
	...
	if(misc_htx_data == NULL){
		printf("misc:fopen failed for %s with retry count=%d,errno=%d\n",str,retries,errno);
	}
	else {
...
/* This function returns PIR value (using sysfs) for passed cpu. */
int get_cpu_id_sysfs(int cpu_no)
{
	FILE *fp;
	int rc,pir = -1;
	char file_name[50];

	sprintf(file_name, "/sys/devices/system/cpu/cpu%d/pir", cpu_no);
...
/* This function returns PIR value for passed cpu. */
int get_cpu_id(void)
{
        int fd, pir;
        ...
        {
			if(misc_htx_data == NULL){
				printf("[%d][%s]misc:Error opening file /dev/miscchar \n",__LINE__,__FUNCTION__);
			}
			else{
...
int htx_bind_thread(int lcpu, int pcpu)
{
	...
	err_num = pthread_setaffinity_np(pthread_self(), size, cpu_mask);
	if(err_num) {
		...
		if(err_num == EINVAL) {
			...
			while(count < 5){
				...
				if(rc){
					...
					if(misc_htx_data == NULL){
						printf("[%d][%s]retrying to bind for pcpu=%d after waiting 60 secs,count = %d\n",__LINE__,__FUNCTION__,pcpu,count);
					}
					else{
...
/* Adding bindprocessor API for maintaing backward portability for hxevmxb hxeflpb hxesysben exercisers. */
int bindprocessor (int What, int Who, int Where)
{
	int rc;

    rc = htx_bind_process(Where, -1);
...
int print_sysfs_cpu(int pcpu,size_t size,cpu_set_t* mask){
	...
	if ( global_only == 0 ) {
		...
		if ( fp1 != NULL )
		{
			...
			if(misc_htx_data == NULL){
				printf("[%d][%s]Failed to open sysfs fp1=%p with errno=%d\n",__LINE__,__FUNCTION__,fp1,errno);
			}
			else{
