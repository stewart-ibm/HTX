static char sccsid[] = "@(#)04  1.19  src/htx/usr/lpp/htx/lib/pdiagex/lib_exer.c, libdiag_lx, htxubuntu 4/4/08 04:45:11";
...
int pdiag_restore_bars(char *device)
{
    ...
    if(!fd_flag) {
        fd = open("/dev/pdiagchar", O_RDWR);
        if (fd < 0) {
            printf("\nerror opening file in user fn pdiag_restore_bars \n");
            return -1;
        }
    }
    rc = ioctl(fd, PDIAGEX_RESTORE_BARS, device);
    if(!fd_flag) {
        close(fd);
        fd = -1;
    }
