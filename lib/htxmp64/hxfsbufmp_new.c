 ...
 */
/* IBM_PROLOG_END_TAG */
static char sccsid[] = "@(#)75  1.2  src/htx/usr/lpp/htx/lib/htxmp64/hxfsbufmp_new.c, htx_libhtxmp, htxubuntu 10/8/10 04:38:42";
/*
 ...
 *      hxfmsg_tsafe instead of hxfmsg. This was done to make hxfupdate thread safe.
 *
 *      Return Codes:
 *      ----------------------------------------------------------------------
      ...
      if ((num_bytes = write(fileid, buf, (int) len)) == -1)
        {
          ...
          (void) hxfmsg_tsafe(ps, exit_code, 1, err_msg);
        }
      else                       /* write() OK                               */
        {
