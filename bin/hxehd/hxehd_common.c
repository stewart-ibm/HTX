/* @(#)37   1.11.1.92  src/htx/usr/lpp/htx/bin/hxehd/hxehd.c, exer_hd, htx71J 9/5/12 03:45:24 */
/******************************************************************************
 ...
 *               Deepthi   | 05/07/10 | Addition of reuse_buffers as a rule
 *	                       |          | file parameter in case of Seq opers
 *               Piyush    | 04/13/11 | Added two separate validation & performance
 *                         |	      |	mode in exerciser .
 *               Piyush    | 24/05/11 | Added new set of operation to exerciser.
 *               Piyush    | 02/11/11 | Enable passthru ioctl's for SAS adapters.
 *               Piyush    | 16/11/11 | Add SIGTERM handler to hxehd for graceful exit.
 *				 Piyush    | 08/06/12 | Increase MAX_THREADS = 512 and MAX_STANZAS = 1024
 *				 Piyush	   | 18/10/12 | Support for 64 bit LBA Number.
 *			     Piyush    | 24/07/13 | Code Common AIX/Linux.
 *				 Piyush	   | 16/10/13 | Add Support for Dynamic Tiered RAID Arays.
 ****************************************************************************/
...
int get_dev_fn(unsigned char *dev_name);

int             rule_stanza[MAX_STANZA], saved_num_blocks;
unsigned long long lun_id =0, scsi_id =0;

#ifndef __HTX_LINUX__
unsigned long long scsi_id_parent, lun_id_parent;
int		pass_thru = 0, debug_passthru = 0;
char        disk_parent[20];
struct          CuDv cusobj;
struct          CuAt *catt;
struct 			CuPath cupath;
char        lv_devices[100][10]; /* for LA trigger */
char        PCI_parent[100][10];
int         PCI_devfn[100];
int         num_of_processors_allowed, cpu_bind = 0, cpustate = 0, tmp_cpustate = 0;
int         count_active_cpus, arch;
#else
char fsync_flag ='N';
#define ONE_MB_BITS 20
#define ONE_MB_MASK (1UL << ONE_MB_BITS)
#define ONE_MB      (ONE_MB_MASK - 1)
#define ONE_BYTE    8
#endif
...
int             saved_data_len, seeds_were_saved = 0, page_align = 0, bufrem = 0;
int             hang_time = 600, threshold = 3, mode = VALIDATION;
int      read_rules_file_count;
int volatile    cache_cond, threads_created;
int volatile    number_of_threads, percent;
extern unsigned long long volatile  collisions, lba_fencepost;
static unsigned long long volatile saved_maxlba;
...
char            hostname[10], rules_file_name[100];
char volatile   backgrnd_thread = 'F';
char            int_signal_flag = 'N';
FILE            *tptr;
time_t          time_mark;
...
/******************************************************************************
 * Function to set the signal flag to yes when a signal INT is received by the
 * exerciser. Used with PIPES.
 *****************************************************************************/
...
main (int argc, char *argv[])
{
	...
	int              done, i, rc, stanza_num;
	unsigned long long  maxblk = 0;
	char             bad_rule, work[100];
	...
	#ifdef __HTX_LINUX__
	/* Linux Specific Variable declaration */
	#else
	/* AIX Specific Variable declaration. */
	char * d_name;
	#endif
    ...
    sigaction(SIGINT, &sigdata, NULL);            /* call when recv sig 30 */
	...
	/* Begin code                   */
	memset(&s, 0, sizeof(struct htx_data));
	...
	if ( s.run_type[0] != 'M' ) {
		sprintf(msg, "%s %s %s %s \n", s.HE_name, s.sdev_id, s.run_type, rules_file_name);
		user_msg(&s, 0, INFO, msg);
	}
	i = strlen(s.sdev_id);
	done = 0;
...
#else
	sprintf(cmd_str, "/usr/lpp/htx/etc/scripts/check_disk %s", s.sdev_id);
	rc = system(cmd_str);
	if (WIFEXITED(rc)) {
		if (WEXITSTATUS(rc) == 1) {
	...
	if(eeh_env != NULL) {
		eeh_enabled = atoi(eeh_env);
	}
	if ( eeh_enabled ) {
		eeh_retries = atoi(getenv("HTXEEHRETRIES"));
	} else if((strcmp(s.run_type, "REG") == 0) && s.p_shm_HE->cont_on_err) {
		/* If User specifies COE in mdt file, then i need to make sure
	 	 * all read/write call are successfull.
	 	 * If we see ocassional, error retry
	 	 */
		eeh_enabled = 1;
		eeh_retries = 5;
	}

	sprintf(msg, "eeh_enabled = %d , eeh_retries = %d \n", eeh_enabled, eeh_retries);
	user_msg(&s, 0, INFO, msg);

	r.fildes = eeh_open(&s,s.sdev_id, O_RDONLY);  /* Open device */
	if ( r.fildes == -1 ) {
	...
	#ifdef __HTX_LINUX__
		/* Hxehd own's IOCINFO IOCTL, check hd_linux.c for implementation */
		rc = htx_ioctl(r.fildes, IOCINFO, &info);
	#else
		rc = eeh_ioctl(&s, r.fildes, IOCINFO, &info); /* get device information */
	#endif
	if (rc == -1) {
		sprintf(msg, "IOCTL IOCINFO error in MAIN - ");
		user_msg(&s, errno, HARD, msg);
		if ( (close(r.fildes)) == -1 ) {
			sprintf(msg, "CLOSE call failed in MAIN -");
			user_msg(&s, errno, HARD, msg);
		}
		exit(1);
	}
	#ifdef __HTX_LINUX__
	maxblk = (unsigned long long)info.un.scdk.numblks;
   	blksize = (unsigned int) info.un.scdk.blksize;
	sprintf(msg, "Device %s Information:  \n" "BlockSize = 0x%x,  Number of Blocks = 0x%llx\n",
                s.sdev_id,  blksize, maxblk);
    user_msg(&s, 0, INFO, msg);
    /* Now if maxblk unintialized then its error, better report and exit !! */
    if(maxblk == 0) {
        sprintf(msg,"Not able  detect num_blks on disk = %s, maxblk = 0x%llx \n",s.sdev_id, maxblk);
        user_msg(&s, 0, HARD, msg);
        exit(1);
    }
	...
	if ( info.devtype == DD_SCDISK || info.devtype == DD_SCRWOPT || info.devtype == DD_CDROM)  {
		...
		if ( rc ) {
			...
			if (info.devtype == DD_CDROM ) {	/* SCSI or IDE CDROMs */
				maxblk = (unsigned int)info.un.sccd.numblks;
				blksize = (unsigned int)info.un.sccd.blksize;
				if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
					maxblk = ((unsigned long long)info.un.sccd64.hi_numblks << 32) | ((unsigned long long)info.un.sccd64.lo_numblks & 0xffffffffULL);  /* yes */
                    blksize = info.un.sccd64.blksize;
				}
			}
			else {  /* SCSI or IDE disks */
				maxblk = (unsigned int)info.un.scdk.numblks;
				blksize = (unsigned int) info.un.scdk.blksize;
				if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
					maxblk = ((unsigned long long)info.un.scdk64.hi_numblks << 32) | ((unsigned long long)info.un.scdk64.lo_numblks & 0xffffffffULL);  /* yes */
                    blksize = info.un.scdk64.blksize;
				}
			}
			sprintf(msg, "Device %s Information:  \nType=%#x, Flag=%#x, SCSI_ID = %#llx,  LUN = %#llx, \nBlockSize = 0x%x,  Number of Blocks = 0x%llx\n",
			    s.sdev_id, info.devtype, info.flags, scsi_id, lun_id, blksize, maxblk);
			user_msg(&s, 0, INFO, msg);
		}
	} else { /* LVs */
		maxblk = (unsigned int)info.un.dk.numblks;
		blksize = (unsigned int)info.un.dk.bytpsec;
		if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
			maxblk = ((unsigned long long)info.un.dk64.hi_numblks << 32) | ((unsigned long long)info.un.dk64.lo_numblks & 0xffffffffULL);  /* yes */
			blksize = info.un.dk64.bytpsec;
        }
		sprintf(msg, "Device %s Information: \n" "Type=%#x, Flag=%#x, \nBlockSize = %#x,  Number of Blocks = %#llx\n"
		    , s.sdev_id, info.devtype, info.flags, blksize, maxblk);
		user_msg(&s, 0, INFO, msg);
	}
	/* Now if maxblk unintialized then its error, better report and exit !! */
    if(maxblk == 0) {
	...
	if ( (eeh_close(&s, r.fildes)) == -1 ) {
		sprintf(msg, "CLOSE call failed after collecting disk info");
		user_msg(&s, errno, HARD, msg);
		exit(1);
	}
	...
	if ( rc != 0 ) {
		sprintf(msg, "Check the HTXERR log for a listing of errors with the " "rules file! RC = %d\n", rc);
		user_msg(&s, 950, HARD, msg);
		exit(1);
	}
	...
	pthread_create(&(threads[0]), &thread_attrs_detached, (void *(*)(void *)) run_timer, (void *)(&s));
	do {
		collisions = 0;
		stanza_num = 0;
		threads_created = 1;
		...
		while ( 1 ) {
			if ( current_ruleptr == NULL ) {
				if ( backgrnd_thread == 'F' )
					...
					sprintf(msg, "Pass #%d of rule file %s completed.\n" "Collision count = %d." "  Back_Ground thread is %d percent complete!\n",
					    read_rules_file_count, rules_file_name, collisions, percent);
					user_msg(&s, 0, INFO, msg);
					hxfupdate(FINISH, &s);
			...
			hxfupdate(UPDATE, &s);
			if ( rc == 0 ) {
				if(strcmp(current_ruleptr->oper, "S") == 0) {
					rc = usleep(current_ruleptr->sleep);
				} else if((strncmp(current_ruleptr->oper, "X", 1) == 0)) {
					rc = do_cmd(&s, current_ruleptr);
				#ifndef __HTX_LINUX__
				} else if((strncmp(current_ruleptr->oper, "OM", 2) == 0)) {
					...
					if ( (backgrnd_thread == 'R') && (strcmp(current_ruleptr->oper, "BWRC") == 0) ) {
						...
						if ( (rc = pthread_create(&(threads[threads_created]), &thread_attrs_detached,
						    (void *(*)(void *)) zip_thread, (void *)(&rule_stats)))) {
							sprintf(msg, "rc %d, errno %d from main(): pthread_create", rc,
							    errno);
							user_msg(&s, 950, HARD, msg);
							exit(rc);
						...
						if ( current_ruleptr->start == 'Y' ) {
							...
							while(number_of_threads > 0  && number_of_threads < MAX_THREADS )
								sleep(5);
							/* Check if we have come out because of SIGTERM, Just break off the loop */
							if(exit_flag == 'Y')
								break;
							if ( number_of_threads != 0 ) {
								sprintf(msg, "Error with Threads: Number_of_Threads = %d\n"
								    "Threads_Created = %d, Backgrnd_Thread = %c," " Percent = %d\n",
								    number_of_threads, threads_created, backgrnd_thread, percent);
								user_msg(&s, 950, HARD, msg);
							}
			...
			if ( ((s.run_type[0] == 'M') || (s.run_type[0] == 'O')) && rc != 0  || exit_flag == 'Y')
				break;
			/************************************************************************/
			...
			if ( signal_flag == 'Y' ) {
				rc = get_rule(&s, rules_file_name, maxblk, blksize);
				if ( rc != 0 ) {
					sprintf(msg, "Check the HTXERR log for error information on " "the pipe rules file!\n");
					user_msg(&s, 950, HARD, msg);
					return(1);
				} else
					current_ruleptr = first_ruleptr;
			}
            if(int_signal_flag=='Y')
                break;
		}
		sprintf(msg, "Pass #%d, rule file %s completed.\nCollision count = %d.", read_rules_file_count, rules_file_name,
		    collisions);
		user_msg(&s, 0, INFO, msg);
		hxfupdate(FINISH, &s);
		read_rules_file_count++;
	...
	} while ( ( strcmp(s.run_type, "OTH") != 0 ) && (int_signal_flag == 'N'));
	int_signal_flag='N';
...
start_msg(struct htx_data *ps, struct ruleinfo *pr, char *msg_text)
{
	...
	if(pr->num_oper == 0) {
		total_blocks = pr->max_blkno;
	} else {
		total_blocks = pr->num_blks * pr->num_oper;
	}

	if ( strcmp(pr->addr_type,"SEQ") == 0 )
		sprintf(msg_text, "Started %s pattern_id = %s sleep = %d\n" "addr_type = %s num_oper = %d oper = %s\n" "starting_block = %s direction = %s increment = %d\n"
		    "type_length = %s num_blks = %d total blks = %#llx", pr->rule_id, pr->pattern_id, pr->sleep, pr->addr_type,
		    pr->num_oper, pr->oper, pr->starting_block, pr->direction, pr->increment, pr->type_length, pr->num_blks,
		    total_blocks);
	else
		sprintf(msg_text, "Started %s pattern_id = %s sleep = %d\n" "addr_type = %s num_oper = %d oper = %s\n" "type_length = %s num_blks = %d tot_blks < %#llx",
		    pr->rule_id, pr->pattern_id, pr->sleep, pr->addr_type, pr->num_oper, pr->oper, pr->type_length, pr->num_blks,
		    total_blocks);
...
proc_rule(struct htx_data s, struct ruleinfo r)
{
	...
	unsigned int alignment = 0, nreads = 0, nwrites = 0, opertn = 1 ; /* Assume we are going to do 1 IO operation in 1 num_oper */
	unsigned int hot = 0;
	volatile unsigned short write_stamping = 0;

	#ifdef __HTX_LINUX__
		/* Linux specific Variable declaration */
		struct timespec op_rate;
	#else
		/* AIX Specific variable declaration */
	...
	ps = &s;
	pr = &r;
    bzero(wptr,MAX_BUFFERS*sizeof(char *));
    bzero(rptr,MAX_BUFFERS*sizeof(char *));
	...
	if(pass_thru == 0) {
		...
		#ifdef __HTX_LINUX__
		    if( (strchr(pr->oper, 'W') != NULL) || (strchr(pr->oper, 'C') != NULL) ) {
        		pr->fildes = eeh_open(ps, ps->sdev_id, O_RDWR | O_DIRECT);
    		} else {
        		pr->fildes = eeh_open(ps, ps->sdev_id, O_RDONLY | O_DIRECT);
    		}
		#else
			if( strchr(pr->oper, 'W') != NULL ) {
				pr->fildes = eeh_open(ps, ps->sdev_id, O_RDWR);
			} else {
				pr->fildes = eeh_open(ps, ps->sdev_id, O_RDONLY);
			}
		#endif
			if ( pr->fildes == -1 ) {
				if (errno == EINVAL) {
		...
		/*
     	 ...
     	 * PARENT DEVICE, OPEN THE ADAPTER HERE ...
	 	 */
		char tmp[256];
        if (debug_passthru == 1)
        {
        ...
        /* Issue SCIOSTART now */
        if(parent_info.devsubtype == DS_SAS) {

			memset(&sciolst, 0, sizeof(struct scsi_sciolst));
           	sciolst.version = SCSI_VERSION_1;
           	sciolst.scsi_id = scsi_id;
           	sciolst.lun_id = lun_id;
           	sciolst.flags = (ISSUE_LOGIN | FORCED);
            rc = eeh_ioctl(ps, adapter, (uint)SCIOLSTART, (char *)&sciolst);
			if(rc != 0) {
				close(adapter);
				sprintf(msg, "SCIOLSTART failed: %s\n",sys_errlist[errno]);
 	            user_msg(ps, errno, HARD, msg);
    	        return(-1);
           	}
        } else {
            rc = eeh_ioctl(ps, adapter, SCIOSTART, (void *)IDLUN(scsi_id_parent, lun_id_parent));
            if( rc !=0 ) {
                close(adapter);
		...
		/* Opening of adapter successful, lets save the file descriptor */
		pr->adapter = adapter;
	}
	#endif
	...
	case DD_SCRWOPT:
		#ifndef __HTX_LINUX__
		pr->bytpsec   = (unsigned int)info.un.scdk.blksize;
		pr->tot_blks  = (unsigned int)info.un.scdk.numblks;
		if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
        	pr->tot_blks = ((unsigned long long)info.un.scdk64.hi_numblks << 32) | ((unsigned long long)info.un.scdk64.lo_numblks & 0xffffffffULL);  /* yes */
            pr->bytpsec  = info.un.scdk64.blksize;
        }
        #else
        pr->bytpsec   = (unsigned long long)info.un.scdk.blksize;
		pr->tot_blks  = (unsigned long long)info.un.scdk.numblks;
		#endif
		break;
	#ifndef __HTX_LINUX__
	case DD_CDROM:
		pr->bytpsec   = (unsigned int)info.un.sccd.blksize;
		pr->tot_blks  = (unsigned int)info.un.sccd.numblks;
		if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
        	pr->tot_blks = ((unsigned long long)info.un.sccd64.hi_numblks << 32) | ((unsigned long long)info.un.sccd64.lo_numblks & 0xffffffffULL);  /* yes */
           	pr->bytpsec = info.un.sccd64.blksize;
        }
		break;
	default:
		pr->bytpsec   = (unsigned int)info.un.dk.bytpsec;
		pr->tot_blks  = (unsigned int)info.un.dk.numblks;
		if(info.flags & DF_LGDSK ) { /* is disk > 32-bit LBA range (2TB)? */
            pr->tot_blks = ((unsigned long long)info.un.dk64.hi_numblks << 32) | ((unsigned long long)info.un.dk64.lo_numblks & 0xffffffffULL);  /* yes */
            pr->bytpsec = info.un.dk64.bytpsec;
        }
	#endif
	} /* end switch */
	if(pr->tot_blks == 0) {
        sprintf(msg,"Not able  detect num_blks on disk = %s, maxblk = 0x%llx \n",ps->sdev_id, pr->tot_blks);
        user_msg(&s, 0, HARD, msg);
        return(1);
	}
	...
	/*
 	 ...
 	 * or if user has specified max_blkno < num_blks in the stanza, take
 	 * the minimum of the two, to be safe.
 	 */
	...
	if ( strcmp(pr->oper, "BWRC") == 0 ) {
		...
		if ( pr->om_prevent == 0 ) {      /* if user did not specify max blkno */
			...
			prcnta = (double)pr->tot_blks;
		} else /* else use the user specified blkno */
			prcnta = (double)pr->max_blkno;
	}
	/*
	...
	if ( (strcmp(pr->oper, "BWRC") != 0) && (is_bwrc_rule_present == 1)) {
      	/*
         * If BWRC is damn slow and its not able to complete num_blks before this stanza starts then better hold
         * this stanza from running.
         */
		while ( (backgrnd_thread == 'R') && ((lba_fencepost == 0) || (pr->num_blks > lba_fencepost)))
	...
	if ( pr->min_blkno >= pr->max_blkno ) {
		if ( backgrnd_thread == 'E' ) {
			sprintf(msg, "This stanza %s will not be run as MIN LBA %#llx " "is greater than the MAX LBA %#llx.\nThe cause "
			    "of this error is due to a HARD error in the " "background thread.\n", pr->rule_id, pr->min_blkno,
			    pr->max_blkno);
			user_msg(ps, 0, HARD, msg);
			rc = -1;
		} else {
			sprintf(msg, "This stanza %s will be skipped as MIN LBA %#llx" "\nis greater than the MAX LBA %#llx",
			    pr->rule_id, pr->min_blkno, pr->max_blkno);
			user_msg(ps, 0, INFO, msg);
			rc = 0;
		}
		if ( (eeh_close(ps, pr->fildes)) == -1 ) {
			sprintf(msg, "CLOSE call failed!");
			user_msg(ps, errno, HARD, msg);
			exit(1);
		}
		return(rc);
	...
	if ( pr->min_blkno > pr->tot_blks ) {
		...
		sprintf(msg, "In this stanza %s: STARTING LBA %#llx was less than MINIMUM " "LBA %#llx\nSTARTING LBA will be reset to MINIMUM LBA!",
	...
	if ( strcmp(pr->addr_type, "SEQ") == 0 )
    	...
    	*************************************************************************/
		if ( (pr->rule_options & RESTORE_SEEDS_FLAG) && (strcmp(pr->type_length, "FIXED") == 0) )
			random_blkno(blkno, saved_data_len, pr->bytpsec, pr->max_blkno, &seed, pr->min_blkno, pr->lba_align);
		else /* We're not restoring seeds, so use the current data length */
			random_blkno(blkno, pr->dlen, pr->bytpsec, pr->max_blkno, &seed, pr->min_blkno, pr->lba_align);
	}

	/* Check if user specified a seed value for this rule.  Indicated by pattern
     ID of !001xxx or !002xxx. */
	if ( pr->pattern_id[0] == '!' ) {
		if ((strncmp(pr->pattern_id + 1, "001", 3) != 0 && strncmp(pr->pattern_id + 1, "002", 3) != 0) ||
			  strlen(pr->pattern_id) != 7 ) {
			sprintf(msg, "pattern error - Fixed seed (\"%s\") patterns must have form:\n" "!001xyz or !002xyz where xyz is used as the seed.\n",
			    pr->pattern_id);
			user_msg(ps, 950, HARD, msg);
			if((eeh_close(ps, pr->fildes)) == -1 ) {
				sprintf(msg, "CLOSE call failed!");
	...
	******************************************************************************/
	...
	if ( pr->num_oper == 0 || (strncmp(pr->rule_id, "CLEAN", 5) == 0)) {
		...
		if(strchr(pr->oper, '[') == NULL)
			...
			if(pr->opertn.compare_enabled) {
				opertn = pr->opertn.num_writes;
			} else {
				opertn = ((pr->opertn.num_writes > pr->opertn.num_reads) ? pr->opertn.num_writes : pr->opertn.num_reads);
			}
			pr->num_oper = rc / ((pr->num_blks + pr->increment) * opertn);
		}
		if ( rc % ((pr->num_blks + pr->increment) * opertn)) {
			pr->num_oper++;
			do_partial = 1;
		}
	} else {
		if ( pr->percent > 0.0 ) {
			/**********************************************************************
       		...
       		***********************************************************************/
			pr->num_oper = ( (pr->max_blkno - pr->min_blkno) / pr->num_blks ) * pr->percent;
	...
	if (wrap(pr, blkno) ) {
		if (do_partial ) {
			if( *pr->direction == 'U') {  /* UP direction */
				...
				/* Not sure why we need to change LBA ?? -- Piyush
			...
			sprintf(msg,"Partial xfer required on loop %d starting at\n" "block number %d (0x%x).  Transfer length will be\n"
			    "%d (0x%x) blocks (%d bytes).", loop+1, blkno[0], blkno[0], pr->num_blks, pr->num_blks, pr->dlen);
			user_msg(ps, 0, INFO, msg);
	...
	/* For type_len = FIXED, max we are writing is pr->num_blks * pr->bytpsec.
	 * For type_len = RANDOM, max we are writing is (pr->num_blks + 1) * pr->bytpsec.
	 */
    pattern_buffer = (char *)malloc((pr->num_blks + 1) * pr->bytpsec * sizeof(char) + 128);
	...
	/***************************************************************************
	 * These variables are thread global and would be used for specific pattern
	 * generatrion. Used in Pattern 8,9 and A.
    ...
    if ( pr->pattern_id[0] != '#' && pr->pattern_id[0] != '!' ) {
        ...
        rc = hxfpat(path, pattern_buffer, (pr->num_blks + 1) * pr->bytpsec * sizeof(char) + 128);
        if ( rc == 1 ) {
        ...
        pr->rand_index = 0;
        pr->and_mask = 0x1;
        pr->or_mask = 0x8000000000000000ULL;
        pr->beging_dword = 0;
        if(pr->align > 0) {
            pr->trailing_dword = ((pr->align / sizeof(unsigned long long)) -1);
        } else {
            pr->align  = 16;
            pr->trailing_dword = ((pr->align / sizeof(unsigned long long)) -1);
        }
		 ...
		 * random numbers in num_oper we offset and pick numbers from
		 * here
		 */
		pr->size_rand_buf = ((pr->num_blks + 1) * pr->bytpsec * sizeof(char) + 128);
		rand_buffer = (unsigned int *)pattern_buffer;
		pr->rand_buf = (char *)rand_buffer ;

		for(i = 0; i < (pr->size_rand_buf/sizeof(unsigned int)); i++) {
			*(unsigned int *)rand_buffer = (unsigned int)get_random_number(&seed);
			rand_buffer++;
		}
	}
	...
	if(mode == PERFORMANCE) {
	    /*********************************************************************
 	     * Moved malloc's out of num_oper loop, use pre canned write buffers
		 * For max thru put : addr_type = Fixed, type_length = fixed,
		 * pattern_id = HEX255.
     	 ********************************************************************/
		for(mallocount = 0; mallocount < pr->no_mallocs; mallocount++) {
           	if( pr->align > 0 ) { /* If align us specified then IO buffers are aligned to this */
               	alignment = pr->align;
           	} else if(pr->lba_align) { /* If not specified, Check if LBA align is specfied */
 	       	...
 	       	wptr[mallocount] = (char *) malloc(((pr->num_blks + 1) * pr->bytpsec ) * sizeof(char) + alignment + 128);
    	    rptr[mallocount] = (char *) malloc(((pr->num_blks + 1) * pr->bytpsec ) * sizeof(char) + alignment + 128);
            ...
            strt_wptr[mallocount] = wptr[mallocount];
            strt_rptr[mallocount] = rptr[mallocount];

        	/* Adjust start wptr and rptr on the basis of alignment
         	 * requested
			 */
        	if(alignment) {
            	bufrem = ((unsigned)strt_wptr[mallocount]) % (alignment);
            	if ( bufrem != 0 )
                	strt_wptr[mallocount] = strt_wptr[mallocount] + (alignment - bufrem);
            	bufrem = ((unsigned)strt_rptr[mallocount]) % (alignment);
            	if( bufrem != 0)
                	strt_rptr[mallocount] = strt_rptr[mallocount] + (alignment - bufrem);
        	}
            if ( pr->pattern_id[0] != '#' && pr->pattern_id[0] != '!' ) {
                if(strcmp(pr->type_length, "FIXED") == 0) {
                    /* initialize write buffer if static pattern specified
                     */
                    memcpy(strt_wptr[mallocount], pattern_buffer, ((pr->num_blks + 1) * pr->bytpsec  + 64) * sizeof(char)); /* max pr->dlen + offset = 63 */
				}
			}
    	}
	}
   	mallocount = 0;
	pr->offset = 0;
	if ( pr->timed == 'Y') {  /* Clock the time taken to finish this stanza */
#ifndef __HTX_LINUX__
	...
	for (loop = 1; loop <= pr->num_oper; loop++ ) {
		...
		/* Check if SIGTERM Recvd ... */
		if(exit_flag == 'Y') {
			/** CleanUp...... **/
			break;
		}
		if(mode == VALIDATION) {
			/* Malloc a new buffer each time, this allows to post new buffer each time
			 * for read/write, allows us to get new DMA mapping, adding to DMA stress.
			 */
            if( pr->align > 0 ) { /* If align us specified then IO buffers are aligned to this */
                alignment = pr->align;
       		...
       		wptr[mallocount] = (char *) malloc(pr->dlen + alignment + 128);
       		rptr[mallocount] = (char *) malloc(pr->dlen +  alignment + 128);
			...
			if(pr->offset) {
				...
				wbuf = wptr[mallocount];
				rbuf = rptr[mallocount];
			}

       		/* Adjust  wbuf and rbuf on the basis of alignment
       	 	 * requested
		 	 */
       		if(alignment) {
           		bufrem = ((unsigned)wbuf) % (alignment);
           		if ( bufrem != 0 )
               		wbuf = wbuf + (alignment - bufrem);
           		bufrem = ((unsigned)rbuf) % (alignment);
           		if( bufrem != 0)
               		rbuf = rbuf + (alignment - bufrem);
       		}
		} else {
			/* mode == performance */
		    /* Intialize read/write buffer */
		    if(pr->offset) {
				rbuf = strt_rptr[mallocount] + pr->offset;
            	wbuf = strt_wptr[mallocount] + pr->offset;
			} else {
        		rbuf = strt_rptr[mallocount];
        		wbuf = strt_wptr[mallocount];
			}
		}
        /* Blocks, and dlen changes in each num_oper loop
		if((ps->run_type[0] == 'O' || pr->messages[0] == 'D') &&(loop % 500 ==0) ) {
			info_msg(ps, pr, loop, blkno, msg);
			user_msg(ps, 0, INFO, msg);

			printf("%s:wbuf=%#x, rbuf=%#x, dlen=%#x, num_blks = %#x, bytpsec = %#x, mallocount = %#x, fencepost=%#llx \n",
					pr->rule_id, wbuf, rbuf, pr->dlen, pr->num_blks , pr->bytpsec, mallocount, lba_fencepost);
		...
		*/
		...
		if(strchr(pr->oper, '[') == NULL) {
			...
			/* Changes for Dynamic Tiered  RAID Array, this would do IO on same
			 * set of Bands, so as to make it HOT
			 */
			 for(hot=1; hot <= pr->hotness; hot++) {
        		/* Check if SIGTERM Recvd ... */
        		if(exit_flag == 'Y') {
            		/** CleanUp...... **/
            		break;
 		       ...
 		       if((ps->run_type[0] == 'O' || pr->messages[0] == 'D') ) {
            		...
            		printf("%s:wbuf=%#x, rbuf=%#x, dlen=%#x, num_blks = %#x, bytpsec = %#x, mallocount = %#x, fencepost=%#llx, hot=%#x, hotness=%#x \n",
                    pr->rule_id, wbuf, rbuf, pr->dlen, pr->num_blks , pr->bytpsec, mallocount, lba_fencepost, hot, pr->hotness);

       	 		}


				/* Inside hotness loop following are fixed
				 * 		>> LBA set to which IO is performed.
				 * 	    >> Transfer Size.
				 * We Build Buffer each time in HOT loop to distinguish each Write
				 */
				write_stamping = hot;
				if(mode == VALIDATION) {
					if ( pr->pattern_id[0] != '#' && pr->pattern_id[0] != '!' ) {
            	    	/* initialize write buffer if static pattern specified
                	 	 */
        	        	memcpy(wbuf, pattern_buffer, pr->dlen);
    	        	} else {
                		if(pr->pattern_id[0] == '#' && (loop > 1))
                    		init_seed(&data_seed);
                		/* Overlay the write buffer with Algorithm's specified by pattern_id */
                		if ((strcmp(pr->oper, "R") != 0) && (strcmp(pr->oper, "RS") != 0) )
                    		bldbuf((unsigned short *)wbuf, &data_seed, blkno, ps, pr, pr->pattern_id[3], write_stamping);
            		}
				} else {
					if ( pr->pattern_id[0] != '#' && pr->pattern_id[0] != '!' ) {
                		if(strcmp(pr->type_length, "FIXED") != 0) {
                    		/* initialize write buffer if static pattern specified
                     	 	 * and type_length = RANDOM.
                     	 	 */
                    		memcpy(wbuf, pattern_buffer, pr->dlen);
                		} else {
                    		;
				...
				if ( strcmp(pr->oper, "RWRC") == 0 ) {
					...
					if ( rc == 0 )
						rc = read_disk(ps, pr, loop, blkno, rbuf);
					if ( rc == 0 )
						rc = cmpbuf(ps, pr, loop, blkno, wbuf, rbuf, 1);
					...
					if ( rc == 0 && strcmp(default_patternid, pr->pattern_id) ) {
						...
						if ( default_patternid[0] != '#' && default_patternid[0] != '!' ) {
							...
							bldbuf((unsigned short *)wbuf, &data_seed, blkno, ps, pr, default_patternid[3], write_stamping);
						rc = write_disk(ps, pr, loop, blkno, wbuf);
					}
					...
					rc = read_disk(ps, pr, loop, blkno, rbuf);
					if ( rc == 0 )
						rc = cmpbuf(ps, pr, loop, blkno, wbuf, rbuf, 0);
				}
        	...
        	******************************************************************************/
        	if(backgrnd_thread == 'R') {
            	if (strcmp(pr->oper, "BWRC") == 0)  {
                	/* We need to add num_blks because blkno is the current blkno,
                 	 * prcnta is equated to total blks. Logically 100% finish
                 	 * comes with prcntb is at total blks - num_blks.
                 	 */
                 	prcntb = blkno[0] + pr->num_blks;
                	...
                	if ( (blkno[0] + pr->num_blks + pr->increment) > lba_fencepost &&
                    	...
                    	sprintf(msg, "Stanza %s has caught the background thread!\n" "LBA will be reset - Could cause miscompares!\n"
                        	"Fencepost = %d, LBA before reset = %d\n", pr->rule_id, lba_fencepost, (blkno[0] + pr->num_blks + pr->increment));
                    	user_msg(ps, 0, INFO, msg);
                	}
            	}
        	}
        	...
        	if ( strcmp(pr->addr_type, "RANDOM") == 0 ) {
            	...
            	do_boundary_checks(pr, ps, blkno, &saved_dir_flag, &do_partial);
        	}
		} else {
			...
			for(hot = 1; hot <= pr->hotness; hot ++) {
				...
				/* We need to do N reads/M writes in each loop.
			 	 * For these set of operations
			 	 * operating blocks numbers can vary
			 	 * dlen remains same for these operation, will change only after
			 	 * we complete one set of operations.
			 	 */
				nreads  =  pr->opertn.num_reads;
				nwrites = pr->opertn.num_writes;
				opertn  = (nreads > nwrites ) ? nreads : nwrites;

				/*
				 *	Ensure the Read-Write Ratio on disk from rules file
				 *  This would actually screw up.
				 *
				 *	if(backgrnd_thread == 'R') {
				 *		if(nreads > 0)
				 *			nreads = nreads - 1;
				...
				for(i = 0; i < opertn ; i++) {
		 	 	 	if((ps->run_type[0] == 'O' || pr->messages[0] == 'D') ) {
	                    ...
	                    printf("%s:wbuf=%#x, rbuf=%#x, dlen=%#x, num_blks = %#x, bytpsec = %#x, mallocount = %#x, fencepost=%#llx, hot=%#x, hotness=%#x, i=%#x, opertn=%#x \n",
    		                pr->rule_id, wbuf, rbuf, pr->dlen, pr->num_blks , pr->bytpsec, mallocount, lba_fencepost, hot, pr->hotness, i, opertn);
   	        		...
   	        		if(mode == VALIDATION) {
    		    	    if ( pr->pattern_id[0] != '#' && pr->pattern_id[0] != '!' ) {
    	        	 	    /* initialize write buffer if static pattern specified
                   			...
                   			if(pr->pattern_id[0] == '#' && (loop > 1))
                       			init_seed(&data_seed);
                   			/* Need to build buffer only when i am writing to disk, else RC is done on last written  buffer */
							if (nwrites > 0 || is_bwrc_rule_present ) {
								write_stamping = (((hot & 0x7FF) << 5 ) | (nwrites & 0x1F));
                       			bldbuf((unsigned short *)wbuf, &data_seed, blkno, ps, pr, pr->pattern_id[3], write_stamping);
							}
                		}
            		} else {
					...
					seg_lock(ps, *blkno, *blkno + (pr->dlen / pr->bytpsec));
					if(strcmp(pr->opertn.oper, "RW") == 0) {
						if(nreads > 0) {
            	            rc = read_disk(ps, pr, loop, blkno, rbuf);
                	        if(rc == 0 ) {
                    	        nreads --;
								if(pr->opertn.compare_enabled) {
									rc = cmpbuf(ps, pr, loop, blkno, wbuf, rbuf, nwrites);
									if(rc)
										break;
								}
                        	} else
					...
					if(backgrnd_thread == 'R') {
						...
						/* BWRC is not running and only if RC are left, then the left over RC would be done at
						 * at LBA # where last write was done
						 */
						if((is_bwrc_rule_present == 0) &&	/* BWRC rule is not present */
							(nwrites == 0) &&      			/* Writes Exhausted 		*/
						   	(nreads > 0)  &&  		  		/* Reads are left over 		*/
						   	pr->opertn.compare_enabled) { 	/* And we gonna compare 	*/
							/* Done change LBA # */
							continue;
						}
					}
                    ...
                    /* Only the last iteration, when (hot = pr->hotness),
                     * is allowed to bypass below "if" rest all will just loop back
                     */
                    if(pr->hotness && (hot < pr->hotness)) {
                        continue;
                    }
					...
					/* Set the blkno for next iteration */
		        	if ( strcmp(pr->addr_type, "RANDOM") == 0 ) {
   	             		if ( (pr->rule_options & RESTORE_SEEDS_FLAG) && (strcmp(pr->type_length, "FIXED") == 0) )
                    		random_blkno(blkno, saved_data_len, pr->bytpsec, pr->max_blkno, &seed, pr->min_blkno, pr->lba_align);
                		else
                    		random_blkno(blkno, pr->dlen, pr->bytpsec, pr->max_blkno, &seed, pr->min_blkno, pr->align);
            		} else {
                		if ( saved_dir_flag > 0 ) {         /* this flag is only valid       */
                    		pr->num_blks = saved_num_blks;  /* if the direction is IN or OUT */
                    		pr->    dlen = saved_dlen;      /* this makes sure we don't lose */
 	        ...
 	        /* In order to maintain the read write ratio,
			 * we keep the dlen constant for entire operation.
			 * Change only for next num_oper loop
			 */
    	    if ( strcmp(pr->type_length, "RANDOM") == 0 )
        		random_dlen(pr->bytpsec, pr->max_blkno, &seed, pr);
		...
		if(pr->timed == 'Y' )
			total_bytes = total_bytes + pr->dlen;

		hxfupdate(UPDATE, ps);                /* update htx statistics        */

		if ( pr->loop_on_offset == 1 )
           	pr->offset = (pr->offset +1) % 64;

		if(mode == VALIDATION) {
			/* Malloc new buffer until we reach no_malloc */
			if ( mallocount >= pr->no_mallocs - 1 )
				freebufs(&mallocount, wptr, rptr);
			else
				mallocount++;
		} else {  /* PERFORMANCE, reuse the same buffers */
			mallocount = (mallocount + 1) % pr->no_mallocs ;
		}
		...
		if (pr->rule_time) {
			...
			if (secs >=  (pr->rule_time)) {
				sprintf(msg,"Time spent(%d) in rule %s and rule time(%d), present(%u, %u) \n", secs, pr->
				    rule_id, pr->rule_time, rdt_present.tb_high, rdt_present.tb_low);
	...
	if ( pr->timed == 'Y' ) {
            ...
            sprintf(msg,"Disk : %s, Total Transfer(R/W)= 0x%llx(bytes), Time spent (%d)secs, (%d)nsecs in rule %s\n", ps->sdev_id, total_bytes, secs, nsecs, pr->rule_id);
            strcat(msg, "Rule finished....\n");
            user_msg(ps, 0, INFO, msg);
	}
	#else
	if ( pr->timed == 'Y' ) {
        ...
        if ( timer3 >= 1.0 ) {
            ...
            if ( e_notation((timer3 * ONE_BYTE) / ONE_MB, tim) )
	...
	if(mode == VALIDATION)
		...
		mallocount_temp = pr->no_mallocs -1 ;
	...
	if(pass_thru == 1) {
    	/*
     	...
     	if(parent_info.devsubtype == DS_SAS) {
        	...
        	sciolst.scsi_id = scsi_id;
        	sciolst.lun_id = lun_id;
     		rc = eeh_ioctl(ps, adapter, SCIOLSTOP, &sciolst);
     	} else {
     		rc = eeh_ioctl(ps, adapter, SCIOSTOP, (void *)IDLUN(scsi_id_parent, lun_id_parent));
     	}
     	if( rc !=0 ) {
     		close(adapter);
    		sprintf(msg, "STOP failed: %s\n",sys_errlist[errno]);
    		user_msg(ps, errno, HARD, msg);
    		return(-1);
    	}
    	if((close(adapter)) == -1) {
			sprintf(msg, "CLOSE call failed for parent adapter !");
			user_msg(ps, errno, HARD, msg);
 	       	exit(1);
    	}
	} else
...
freebufs(int *pmallo, char *wptr[], char *rptr[]) {
    int i= 0;
    if(pmallo != NULL && wptr != NULL && rptr != NULL ) {
...
#ifndef __HTX_LINUX__  /*  AIX Specific Functions */
/******************************************************************************
...
int get_lun(struct htx_data *ps, uchar *dev_name)
{
    ...
    if ( (strncmp(cusobj.parent,"usb",3) == 0 ) ||
        ...
        strcpy(&tmp[strlen(dev_path)], disk_parent);
        ...
        rc = eeh_ioctl(ps, parent_fildes, IOCINFO, &parent_info); /* get device information */
        if ( rc == -1 )
        {
            printf("\n ioctl failed for %x[%s], with error=%d(%s)\n", parent_fildes, tmp, errno, strerror(errno));
            return(-6);
        }


        while(eeh_close(ps, parent_fildes) == -1);
        ...
        if(parent_info.devsubtype == DS_SAS) {
            ...
            strcpy(disk_parent, cupath.parent);
            disk_parent[strlen(cupath.parent)] = '\0';
        }
        /*
         ...
         */
        sscanf(tmp, "%llx,%llx", &scsi_id, &lun_id);
        rc = get_parent_lun(disk_parent);
...
#else
char    *usage(char *msg, char *progname) {

    sprintf(msg, "HXEHD, by HTX India\n");
    sprintf(msg + strlen(msg), "usage: %s device_file run_type rules_file\n", progname);

    return msg;
}

int check_disk(const char *dev_name, int fildes, int *part, struct devinfo *info) {

    int len = 0, i = 0, j = 0;
    char    dev_num[10];
    char    device_whole[10];
    int device_part = 0;
    int fd = 0, rc = 0;
    ...
    switch(len) {
        case 8:
            rc = htx_ioctl(fildes, IOCINFO, info);/* get device information */
            return rc;
        default:
            for(i = 0; i <= len; i++)  {
                if(i > 7 && dev_name[i] >= '0' && dev_name[i] <= '9') {
                    dev_num[j++] = dev_name[i];
                    if(i == 8)
                        device_whole[i] = 0;
                } else {
                    device_whole[i] = dev_name[i];
                }
            }
            device_part = atoi(dev_num);
            *part = device_part;
            fd = open(device_whole, O_RDONLY);
            if (fd == -1 ) {
                fprintf(stderr, "Open error in MAIN - ");
                exit(1);
            }
            rc = htx_ioctl(fd, IOCINFO, info);    /* get device information */
            if ( rc == -1 ) {
                fprintf(stderr, "IOCTL IOCINFO error in MAIN - ");
                if ( (close(fildes)) == -1 ) {
