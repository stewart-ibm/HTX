...
/* @(#)19       1.7  src/htx/usr/lpp/htx/bin/hxehd/hxehd_proto.h, exer_hd, htxubuntu 8/7/13 02:05:41 */
...
void seg_lock(struct htx_data *ps, int flba, int llba);
void seg_unlock(struct htx_data *ps, struct ruleinfo *pr, int flba, int llba);
...
int bldbuf(unsigned short *wbuf, struct random_seed_t *seed, unsigned long long *blkno,
	   struct htx_data *ps, struct ruleinfo *pr, char algorithm);
...
int do_compare( struct htx_data *ps, struct ruleinfo *pr, char wbuf[],
				char rbuf[], int crash_flag, int *badsig, int *cksum,
								int loop, unsigned long long blkno );
