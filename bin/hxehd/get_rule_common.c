 ...
 */
/* IBM_PROLOG_END_TAG */
/* @(#)53       1.37  src/htx/usr/lpp/htx/bin/hxehd/get_rule.c, exer_hd, htx71J 6/12/12 04:22:32 */
/******************************************************************************
 *
 ...
 *                         |          | digits from rule_id.
 *             Piyush      | 24/05/11 | Added new Performance mode and new set
 *                         |          | if operations in hxehd.  
 *			   Piyush      | 18/10/12 | 64 bit LBA Number. 
 *			   Piyush	   | 24/07/13 | Code Common AIX/Linux. 
 *			   Piyush	   | 06/09/13 | Report all error conditions with SOFT error. 
 *			   Piyush	   | 17/10/13 | Add support for Dynamic Tiered RAID Array. 
 ******************************************************************************/
#include "hxehd_common.h"
#include "hxehd_proto_common.h"

int num_sub_blks_psect=0;
extern int    page_align, crash_on_miscom, rule_stanza[MAX_STANZA], seeds_were_saved, la_on_miscom;
extern int    crash_on_anyerr, saved_num_blocks, hang_time, threshold, open_retry_count, cpu_bind, mode;
...
int get_rule(struct htx_data *ps, char rules_file_name[100], unsigned long long maxblk, unsigned long blksize)
{
	...
	int        keywd_count, i, j,  cnt, first_line, digitct;
	int align_tmp = 0;
	static int line = 0;
	FILE       *fptr;
	char       *p, s[200], t[200], varstr[80], optswtch = 'N';
	char       digit[4], eof_file = 'N', keywd[80], msg[MSG_TEXT_SIZE];
	char       error = 'n',  pattern_nm[100];
	...
	while ( eof_file != 'Y' ) {
		...
		if ( rec_lgth > 1 ) {
			if ( s[0] != '*' ) {
				...
				#ifdef __HTX_LINUX__
					/* Linux expects our IO buffer to be minimum block align */ 
					current_ruleptr->align                 = blksize;
				#else 
					/* No such limitation on AIX */ 
					current_ruleptr->align                 = 0; 
				#endif 
					current_ruleptr->lba_align			   = 0; 
				...
				if ( (strcmp(keywd, "RULE_ID")) == 0 ) {
					...
					if ( ((strlen(current_ruleptr->rule_id)) < 1) || 
						...
						if ( digitct > 0 ) {
							...
							sprintf(msg, "line# %d %s = %s (NO Stanza number found " "in RULE_ID) \n",
							    line, keywd, current_ruleptr->rule_id);
							user_msg(ps, 0, SOFT, msg);
							error = 'y';
						}
					...
					if ( tminblk > 0 && tminblk < 1 )          /* chg 3 */
						...
						current_ruleptr->min_blkno = (unsigned long long)tminblk;
					if ( current_ruleptr->min_blkno < 0 ) {
						sprintf(msg, "line# %d ( MIN_BLKNO (%d) must be > 0) \n", line, current_ruleptr->
						    min_blkno);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					if ( tmaxblk > 0 && tmaxblk < 1 )           /* chg 3 */
						...
						current_ruleptr->max_blkno = (unsigned long long)tmaxblk;
					...
					if ( current_ruleptr->pattern_id[0] != '#' && current_ruleptr->pattern_id[0] !=
					    '!' ) {
					...
					if ( strcmp(current_ruleptr->messages, "NO")   == 0  || strcmp(current_ruleptr->
					    messages, "YES")  == 0  || strcmp(current_ruleptr->messages, "DBUG") == 0 );
					else {
						sprintf(msg, "line# %d %s = %s (must be YES or NO) \n", line, keywd, current_ruleptr->
						    messages);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					if ( strcmp(varstr, "SAVE_SEEDS") == 0 ) {
						if ( optswtch == 'N' )
							if ( (current_ruleptr->rule_options & RESTORE_SEEDS_FLAG) == 0 ) {
						...
						if ( optswtch == 'N' )
							if ( seeds_were_saved && !(current_ruleptr->rule_options & SAVE_SEEDS_FLAG) ) {
								current_ruleptr->rule_options |= RESTORE_SEEDS_FLAG;
								optswtch = 'R';
							} 
							else if ( current_ruleptr->rule_options & SAVE_SEEDS_FLAG ) {
								sprintf(msg, "line #%d %s = %s (SAVE_SEEDS " "already specified)\n",
								    line, keywd, varstr);
								user_msg(ps, 0, SOFT, msg);
								error = 'y';
							} 
							else {
								sprintf(msg, "line #%d - RESTORE_SEEDS option " "not allowed unless\n"
								    "saved in previous stanza.", first_line);
								user_msg(ps, 0, SOFT, msg);
								error = 'y';
							}
						else if ( optswtch == 'S' ) {
							printf(msg, "line #%d: Save Seeds and Restore Seeds is NOT " "Allowed in the same stanza!\n",
							    line);
							user_msg(ps, 0, SOFT, msg);
							error = 'y';
						} else {
					...
					if ( strcmp(current_ruleptr->addr_type, "SEQ") == 0 || strcmp(current_ruleptr->addr_type,
					    "RANDOM") == 0 ) ;
					else {
						sprintf(msg, "line# %d %s = %s (must be SEQ or RANDOM) \n", line, keywd,
						    current_ruleptr->addr_type);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					if ( strcmp(current_ruleptr->oper, "R")     == 0 || 
						 ...
						 strchr(current_ruleptr->oper, '[') != NULL ) { 	 
							if(strchr(current_ruleptr->oper, '[')) {    
								/* Parse the operation provided, Supported operation type w[M]r[N]c */ 		
								memset(&current_ruleptr->opertn, 0, sizeof(struct operation)); 
								rc = parse_keywds(current_ruleptr->oper, &current_ruleptr->opertn); 
								if(rc == -1) { 
									sprintf(msg, "line# %d %s = %s - must be of format W[M]R[N]c. \n", line, keywd, current_ruleptr->oper); 
								 	user_msg(ps, 0, SOFT, msg);
								 	error = 'y'; 
								} 			
							} 
						...
						#ifndef __HTX__LINUX__
						sprintf(msg, "line# %d %s = %s - must be R,S,V,W,RC,RS,RW,WR,WS," "WRC,\nBWRC,CARR,CARW,CAWR,CAWW,OMEX,OMMV,RWRC,"
						    "WRCW,XCMD,OMINF,OMINI,OMINQ,OMINV,or OMPRE" "separated by White-space character.\n",
						    line, keywd, current_ruleptr->oper);
						#else
						sprintf(msg, "line# %d %s = %s - must be R,S,W,RC,RS,RW,WR,WS," "WRC,\nBWRC,CARR,CARW,CAWR,CAWW,OMEX,OMMV,RWRC,"
						    "WRCW,XCMD,OMINF,OMINI,OMINQ,OMINV,or OMPRE" "separated by White-space character.\n",
						    line, keywd, current_ruleptr->oper);
						#endif
						user_msg(ps, 0, SOFT, msg);
                    ...
                    if ( strcmp(current_ruleptr->direction, "UP") == 0   ||
						...
						sprintf(msg, "line# %d %s = %s (must be UP, DOWN, IN or OUT) \n", line,
						    keywd, current_ruleptr->direction);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					if ( current_ruleptr->num_blks < 1 || current_ruleptr->num_blks > 32768) {
						sprintf(msg, "line# %d %s = %d (must be > 1 and <= 32768) \n", line, keywd,
						    current_ruleptr->num_blks);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
				} else if ( (strcmp(keywd, "HOTNESS")) == 0) { 
					sscanf(s, "%*s %s", varstr);
					current_ruleptr->hotness = atoi(varstr);
					if ( current_ruleptr->hotness < 1 || current_ruleptr->hotness > 32768) { 
						sprintf(msg, "line# %d %s = %d (must be > 1 and <= 32768) \n", line, keywd,
													current_ruleptr->hotness); 
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					if ( current_ruleptr->max_number_of_threads > 8 || current_ruleptr->max_number_of_threads
					    < 2 ) {
						sprintf(msg, "line# %d %s = %d (must be >= 2 and <= 8) \n", line, keywd,
						    current_ruleptr->max_number_of_threads);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					...
					if ( current_ruleptr->no_mallocs < 0 || current_ruleptr->no_mallocs > 100 ) {
						sprintf(msg, "line# %d %s = %d (must be >= 0 and <= 100) \n", line, keywd,
						    current_ruleptr->no_mallocs);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
					}
					...
					} else if ( strcmp(varstr, "MODE") == 0) { 
						sscanf(s, "%*s %*s %s", varstr);
						if(strcmp(varstr, "PERFORMANCE") == 0)  
							mode = PERFORMANCE;
					...
					#ifdef __HTX_LINUX__
                    } else if ( strcmp(varstr, "FSYNC_FLAG") == 0 ) { 
                    
                        sscanf(s, "%*s %*s %s",varstr);
                        if      (strcmp(varstr,"YES")  == 0 )  fsync_flag = 'Y' ;
                        else if (strcmp ( varstr, "NO") == 0 ) fsync_flag = 'N' ;
                        else { 
                            sprintf(msg,"line# %d %s = %s (must be YES or NO ) \n",
                                line, keywd,varstr);
                                user_msg(ps,0,SOFT,msg);
					...
					if ( current_ruleptr->om_target < 0 || current_ruleptr->om_target > current_ruleptr->
					    om_maxslots ) {
						sprintf(msg, "line# %d %s = %d (must be >= 0 and <= %d) \n", line, keywd,
					...
					if ( current_ruleptr->om_target1 < 0 || current_ruleptr->om_target1 > current_ruleptr->
					    om_maxslots ) {
						sprintf(msg, "line# %d %s = %d (must be >= 0 and <= %d) \n", line, keywd,
				...
				#ifdef __HTX_LINUX__
					if( align_tmp >= blksize && ((align_tmp % blksize) == 0)) { 
						current_ruleptr->align = align_tmp;
					}
					else {
						sprintf(msg, "line# %d %s = %d (Linux expects IO Buffers to be minimum block align) \n",
					...
					if( align_tmp >= 0) { 
						...
						sprintf(msg, "line# %d %s = %d ( A positive value  larger  than 0 for align must be specified) \n",
                            line, keywd, current_ruleptr->offset);
                        user_msg(ps, 0, SOFT, msg);
                        error = 'y';
                    }
				#endif 
				} else if((strcmp(keywd, "LBA_ALIGN")) == 0) { 
					sscanf(s, "%*s %s", varstr);
					align_tmp = atoi(varstr);
					printf("align_tmp %d \n",align_tmp); 
					if(align_tmp > 0) { 
						current_ruleptr->lba_align = align_tmp; 
					} else { 
						sprintf(msg, "line# %d %s = %d ( A positive value for lba align must be specified) \n",
                            line, keywd, align_tmp);
                        user_msg(ps, 0, SOFT, msg);
                        error = 'y';
					}
			...
			if(current_ruleptr != NULL)
			{
				...
				if ( getsysvol(sysvol, sizeof(sysvol)) == NULL ) {
					sprintf(msg, "Error getting system volume names from " "file sysdisk in directory /tmp.\n"
				...
				if ( (tvar = current_ruleptr->num_blks + current_ruleptr->increment) < 0 ) {           /* chg 2 */
					sprintf(msg, "line# %d %s = %d (negative increment must be " "less than the number of blocks specified) \n",
				...
				if ( strcmp(current_ruleptr->direction, "OUT") == 0 ) {
					if ( current_ruleptr->first_block !=  (current_ruleptr->min_blkno + (current_ruleptr->
					    max_blkno - current_ruleptr->min_blkno) / 2)) {
						current_ruleptr->first_block = current_ruleptr->min_blkno + (current_ruleptr->
						    max_blkno - current_ruleptr->min_blkno) / 2;
						sprintf(msg, "line #%d - The STARTING_BLOCK has been " "reset to a MIDDLE LBA for this direction\n",
						    first_line);
						user_msg(ps, 0, SOFT, msg);
					}
				}
				if ( (current_ruleptr->rule_options & (SAVE_SEEDS_FLAG | RESTORE_SEEDS_FLAG)) ) {
					if ( strcmp(current_ruleptr->addr_type, "RANDOM") ) {
						sprintf(msg, "Line %d: SAVE/RESTORE seed options must be " "used with\nRANDOM addressing only.",
						    first_line);
						user_msg(ps, 0, SOFT, msg);
						error = 'y';
                        ...
                        if ( !(strcmp(current_ruleptr->pattern_id, "#003") == 0 ||
                                ...
                                sprintf(msg, "Line #%d - When using SAVE/RESTORE_SEEDS option "
                                    ...
                                    "#004, #005, or #006 to avoid\nmiscompares due to "
                                    "overlapping sequences.", first_line);
                                    user_msg(ps, 0, SOFT, msg);
                                    error = 'y';
						} else if ( current_ruleptr->rule_options & SAVE_SEEDS_FLAG ) {
							...
							sprintf(msg, "Line %d - NUM_BLKS in RESTORE_SEEDS stanza " "using\nTYPE_LENGTH = RANDOM must use same NUM_"
							    "BLKS\n as stanza where seeds were saved.", first_line);
							user_msg(ps, 0, SOFT, msg);
							error = 'y';
						}
					} else if ( current_ruleptr->rule_options & SAVE_SEEDS_FLAG ) {
				...
				if (current_ruleptr->lba_align > 0 && ((current_ruleptr->num_blks % current_ruleptr->lba_align) != 0)) { 
					sprintf(msg, "lba_align(%#x) specified is not multiple of num_blks(%#x) \n", current_ruleptr->lba_align, current_ruleptr->num_blks); 
					user_msg(ps, 0, SOFT, msg);
					error = 'y';
                } 
				if ( current_ruleptr->num_oper == 0 && strcmp(current_ruleptr->addr_type, "SEQ") != 0 ) {
					sprintf(msg, "num_oper = 0 but addr_type not SEQ\n");
					user_msg(ps, 0, SOFT, msg);
					error = 'y';
				}
...
static int
parse_keywds( char s[], struct operation * oper ) {

	int len, i, j= 0, k, l = 0; 
	char num[16];
	int w_found = 0, r_found = 0; 
	...
	for(i = 0; i < len; i++) { 
		...
		if(s[i] == '[') { 
			...
			if(k == len)  						
				...
				if(w_found) { 
					w_found = 0; 
					oper->num_writes = atoi(num); 
				} else  
				if(r_found) { 
					r_found = 0; 
					oper->num_reads = atoi(num); 
				} 
			} 	 
		} else  		
		if(s[i] == 'R') { 
	...
	if(oper->num_reads < 0 || oper->num_writes < 0)  
		return(-1); 
