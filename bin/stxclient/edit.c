...
/* @(#)42	1.4.4.1  src/htx/usr/lpp/htx/bin/stxclient/edit.c, eserv_gui, htxubuntu 2/19/15 10:06:54 */
...
void  edit(char *file)
{
  ...
  switch (frkpid) {
    case CHILD:    /* child process                     */
      ...
      strcpy(editor, "/usr/bin/vi");

      if (*file == '\0') {  /* no specified file? run man */
         strcpy(workstr, "-M");

          if (strlen(strcat(workstr, getenv("HTXPATH"))) == 0)  {
           strcat(workstr, "/usr/lpp/htx");
        }

         strcat(workstr, "/man");
        exec_rc = execl("/usr/bin/man", "man", workstr, "htxhp", (char *) 0);
        sleep(5);
      }
      
      else  {
        exec_rc = execl(editor, editor, file, (char *) 0);

      }

      if (exec_rc == -1) {
         sprintf(workstr, "Unable to load %s.  errno = %d", editor, errno);
         printf("Unable to load %s.  errno = %d", editor, errno);
                                 fflush(stdout);
